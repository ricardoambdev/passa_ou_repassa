...
export default function ConfiguracoesPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/magic-word")
      .then((res) => res.json())
      .then((data) => setMagicWord(data.word || ""));
  }, []);
  ...
  async function saveMagicWord() {
    setMagicSaving(true);
    setMagicMsg("");
    try {
      const res = await fetch("/api/admin/magic-word", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ word: magicWord }),
      });
      if (res.ok) {
        setMagicMsg("✅ Palavra salva com sucesso!");
      } else {
        const data = await res.json();
        setMagicMsg("😅 " + (data.error || "Erro ao salvar"));
      }
    } catch {
      setMagicMsg("😅 Erro de conexão");
    }
    setMagicSaving(false);
    setTimeout(() => setMagicMsg(""), 3000);
  }
  ...
  return (
    <div>
      ...
      <div className="space-y-8 max-w-2xl">
        <div className="bg-white rounded-[2rem] shadow-lg border-2 border-zinc-200 p-8">
        ...
        <div className="bg-white rounded-[2rem] shadow-lg border-2 border-zinc-200 p-8">
          <h2 className="text-2xl font-display font-bold text-zinc-700 mb-6">🔐 Palavra Cabalística</h2>
          <p className="text-zinc-500 font-body mb-6">
            Defina a palavra secreta para acessar o sistema. Após digitar corretamente, o sistema não
            pedirá novamente por 24 horas.
          </p>
          ...
          <div className="space-y-4">
            <input
              ...
              placeholder="Digite a palavra cabalística..."
              className="w-full px-4 py-3 rounded-xl bg-orange-50 text-zinc-800 border-2 border-orange-200 focus:border-coral focus:outline-none font-display"
            />
            <button
              onClick={saveMagicWord}
              disabled={magicSaving}
              className="btn-fun px-6 py-3 bg-coral text-white border-coral/80 text-sm disabled:opacity-50"
            >
              {magicSaving ? "💾 Salvando..." : "💾 Salvar Palavra"}
            </button>
          </div>

          {magicMsg && (
            <div className="mt-4 bg-teal/10 text-teal px-4 py-3 rounded-xl font-display text-sm border-2 border-teal/30">
              {magicMsg}
            </div>
          )}
        </div>
